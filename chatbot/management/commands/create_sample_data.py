"""
Django management command to create sample data for the financial chatbot.
"""
...
class Command(BaseCommand):
    help = 'Create sample Excel files and training data for the financial chatbot'
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Creating sample data in: {data_dir}")
        ...
        for filename, data in files_to_create:
            try:
                ...
                self.stdout.write(self.style.ERROR(f"✗ Error creating {filename}: {e}"))
        
        # Create training files
        try:
            # Sample training data - rows
            row_training = """What is the revenue for 2024-25?|revenue|2024-25
...
What is total equity for 2025-26?|equity|2025-26"""
            ...
            column_training = """revenue,income,earnings,sales,turnover
...
current_assets,short_term_assets"""
            ...
            with open(column_file_path, 'w', encoding='utf-8') as f:
                f.write(column_training)
            
            # Data description file
            description = """Financial Dataset Description:

This dataset contains Australian Government Social Services budget data covering:

1. Income Statement (3.1): Revenue, expenses, and net income
2. Balance Sheet (3.2): Assets, liabilities, and equity
3. Changes in Equity (3.3): Equity movements over time
4. Cash Flow Statement (3.4): Operating, investing, and financing activities
...
Key Agencies: DSS, NDIA, NDIS, SAUS, AFIS, DFSV"""
            ...
            self.stdout.write(self.style.SUCCESS("✓ Created training and description files"))
            
        except Exception as e:
            self.stdout.write(self.style.ERROR(f"✗ Error creating training files: {e}"))
        
        self.stdout.write(
            self.style.SUCCESS(
                f"\n🎉 Sample data creation complete! Created {created_files} Excel files."
            )
        )
